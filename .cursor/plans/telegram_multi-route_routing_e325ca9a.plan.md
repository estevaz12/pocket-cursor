---
name: Telegram topic-based agent routing
overview: Route each Telegram forum topic to its own Cursor agent chat (`instance_id` + `pc_id`), with outbound mirroring and inbound sends scoped per topic. Single-user / single-owner semantics unchanged — no multi-user or per-DM tenant model.
todos:
  - id: tdd-baseline
    ...
    status: pending
  - id: route-model
    content: Introduce RouteKey (chat_id + optional message_thread_id), mirrored_chats dict, persistence file + migration from .chat_id/.active_chat (tests for serialization + migration before impl)
    status: pending
  - id: auth
    content: Keep existing owner/pairing; add tests that non-forum DMs behave as today (thread_id None); document forum-only requirement for multiple routes
    status: pending
  - id: cdp-ensure-tab
  ...
  - id: confirms-broadcast
    ...
    status: pending
  - id: docs-env
    content: Document README + .env.example for forum topics → agent mapping (not multi-user)
    status: pending
isProject: false
---

# Telegram forum topics → Cursor agent mapping

## Scope (explicit)

- **In scope:** Multiple **routes** distinguished by `(telegram_chat_id, message_thread_id)` so each **forum topic** in a supergroup can bind to a different Cursor agent chat. Same persistence, monitor fan-out, and CDP tab switching as the fuller design.
- **Out of scope:** Multi-user / multi-tenant Telegram (`TELEGRAM_ALLOWED_USER_IDS`, per-user private routes, “each user picks their own workspace”). **One owner** (existing [`TELEGRAM_OWNER_ID`](.env.example) / auto-pair behavior) remains the model.
...
Incoming DMs already use per-message `cid`; the gap is persistence, per-route mirror state, monitor fan-out per topic, and ensuring sends target the correct Cursor tab and Telegram thread.
...
2. Legacy owner checks unchanged for private chat (`thread_id` None).
...
- **Route key:** `(telegram_chat_id, message_thread_id | None)` — `message_thread_id` distinguishes [forum topics](https://core.telegram.org/bots/api#message) in a supergroup; private DMs and non-topic messages use `None` (same as one global route today).
...
- **No new multi-user allowlist** for this iteration. Keep existing owner pairing.
- **Groups / forums:** Routing multiple agents requires a **forum** supergroup so each topic has a distinct `message_thread_id`. Plain groups without topics cannot separate routes without extra hacks — document this clearly.
- Optional later: **`TELEGRAM_ALLOWED_CHAT_IDS`** if you want to restrict which supergroup may host the bot (bot still only sees messages where [privacy mode](https://core.telegram.org/bots/features#privacy-mode) allows). Not required for topic-only scope.
...
| Area                      | File                                   | Change                                                                                                                                                                                                                                                                                          |
| ------------------------- | -------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Route key                 | [`pocket_cursor.py`](pocket_cursor.py) | New helpers; extend env parsing near lines 81–86 only if needed for optional chat allowlist                                                                                                                                                                                                      |
| Global state              | [`pocket_cursor.py`](pocket_cursor.py) | `mirrored_chat` → `mirrored_chats: dict[RouteKey, ...]`; `last_sent_*` per route                                                                                                                                                                                                                |
| Sender thread             | [`pocket_cursor.py`](pocket_cursor.py) | Resolve route from `msg`; pass route into send path; pairing logic unchanged for owner                                                                                                                                                                                                           |
...
| Docs                      | [`README.md`](README.md)               | Forum topic setup, env vars, privacy mode note — **not** multi-user onboarding                                                                                                                                                                                                                  |
| Example env               | [`.env.example`](.env.example)         | Document any optional chat allowlist only if implemented                                                                                                                                                                                                                                        |
...
- **Cross-topic bleed:** Per-route `last_sent_text` avoids cross-talk for Telegram-origin detection.
...
- **Manual:** Forum supergroup — two (or more) topics, `/chats` in each topic to bind different Cursor agent tabs, verify replies stay in the correct topic with correct `message_thread_id` on send.
