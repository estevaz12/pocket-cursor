...
**Forum topics (multiple agents in one group).** Add the bot to a **forum** supergroup (Topics enabled). Set **`TELEGRAM_FORUM_CHAT_ID`** in `.env` to the group’s numeric id (same format as Telegram `chat.id`, usually `-100…`). The bot must be an **administrator** with **Manage topics**. The bridge then calls Telegram’s `createForumTopic` for each Cursor agent chat when you switch to it (or when a new chat appears), and keeps topic titles in sync with Cursor chat names. You can still use `/chats` to bind manually if needed. Plain groups without topics cannot use per-topic routing. State is stored in `.telegram_routes.json` (plus legacy `.chat_id` / `.active_chat` migration on first run).
