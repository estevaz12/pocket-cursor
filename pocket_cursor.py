...
if routes_file.exists():
    ...
    if _route_bindings_initial:
        try:
            ...
            pass
# Load persisted forum thread remap
...
def _find_forum_route_for_pc(
    ...
    *,
    ...
    Matching is **pc_id-only**: exact ``pc_id`` match is the sole lookup key.
    Prefers same-instance match; falls back to cross-instance same ``pc_id``.
    Fingerprint and chat_name are NOT used for routing (only for display/logging).
    This prevents topic duplication caused by fingerprint divergence across restarts
...
def _ensure_forum_topic_for_cursor_chat(
    ...
    **General**; once ``list_chats`` / the DOM provides a fingerprint we create (or match)
    ...
    with mirrored_chats_lock:
        ...
        _strip_general_mirror_for_chat_unsafe(iid, pc_id)
        # Invariant: at most one forum thread per pc_id.  Drop any stale duplicates.
        for dup_rk in list(mirrored_chats.keys()):
            if (
                dup_rk != rk
                and dup_rk.chat_id == FORUM_CHAT_ID
...
def _recover_stale_forum_topic_after_failed_send(
    ...
    if new_tid is not None:
        ...
        _persist_mirrored_routes()  # Save remap to disk for restart resilience
    return new_tid
...
def _handle_chat_rename(iid, data):
    ...
    Updates ALL mirror rows matching this pc_id -- including cross-instance
    rows -- so the monitor's name-based filter never skips forwarding
    due to a stale chat_name."""
...
def _reconcile_mirrored_chat_from_dom():
    """Sync last_sender_route with the DOM active tab.

    In **forum mode** routes are stable bindings (one thread per Cursor chat).
    We must NOT rebind a route's pc_id -- that would break the mapping and cause
    wrong forwarding.  Instead, find the route whose pc_id matches the active
    DOM tab and point ``last_sender_route`` there.

    In **non-forum mode** (single DM), rebinding the sole route to the active
    tab is the correct behaviour (legacy path, unchanged).
    """
    global mirrored_chats, last_sender_route
    ...
    # -- Non-forum mode: rebind the single route to the active tab (legacy) --
...
def overview_thread():
    ...
    while True:
        try:
            ...
            # Periodic invariant: dedupe forum threads (catches accumulated drift)
            if FORUM_CHAT_ID is not None and _scan_cycle % 10 == 0:
                _dedupe_forum_thread_mirrors_by_cursor_chat()
